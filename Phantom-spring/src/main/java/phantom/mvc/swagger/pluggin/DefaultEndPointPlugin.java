...
import phantom.mvc.data.Result.ResultEnum;
...
public class DefaultEndPointPlugin implements OperationBuilderPlugin {
	...
	public void apply(OperationContext context) {
		...
		if (info == null) {
			log.warn(UString.concat(context.requestMappingPattern(), "的信息未配置"));
			return;
		}
		OperationBuilder builder = context.operationBuilder();
		builder.codegenMethodNameStem((String) UReflect.get(OperationBuilder.class, "summary", builder));
		...
		ModelReference model = (ModelReference) UReflect.get(OperationBuilder.class, "responseModel", builder);
