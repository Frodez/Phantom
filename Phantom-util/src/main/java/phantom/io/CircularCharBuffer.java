 ...
 * </pre>
 *
 * Implements the Circular Buffer producer/consumer model for characters. More
 ...
 * <p>
 * Using this class is a simpler alternative to using a PipedReader and a
 * PipedWriter. PipedReaders and PipedWriters don't support the mark operation,
 * don't allow you to control buffer sizes that they use, and have a more
 * complicated API that requires instantiating two classes and connecting them.
 * <p>
...
public class CircularCharBuffer {
	 ...
	 * writePosition, and the markPosition. If the Reader has never been marked, the
	 * readPosition and the markPosition should always be the same. The characters
	 * available to be read go from the readPosition to the writePosition, wrapping
	 * around the end of the buffer. The space available for writing goes from the
	 * write position to one less than the markPosition, wrapping around the end of
	 * the buffer. The characters that have been saved to support a reset() of the
	 * Reader go from markPosition to readPosition, wrapping around the end of the
	 * buffer.
	 * @since ostermillerutils 1.00.00
	 */
	...
	/**
	 * Number of characters that have to be saved to support mark() and reset() on
	 * the Reader.
	 * @since ostermillerutils 1.00.00
	 */
	 ...
	 * <p>
	 * Note that the number of characters available plus the number of characters
	 * free may not add up to the capacity of this buffer, as the buffer may reserve
	 * some space for other purposes.
	 ...
	 * <p>
	 * Note that the buffer may reserve some characters for special purposes and
	 * capacity number of characters may not be able to be written to the buffer.
	 * <p>
	 * Note that if the buffer is of INFINITE_SIZE it will neither block or throw
	 * exceptions, but rather grow without bound.
	 * @param size desired capacity of the buffer in characters or
	 *            CircularCharBuffer.INFINITE_SIZE
	 * @since ostermillerutils 1.00.00
	 */
	 ...
	 * neither block or throw exceptions, but rather grow without bound.
	 * @param size desired capacity of the buffer in characters or
	 *            CircularCharBuffer.INFINITE_SIZE
	 * @param blockingWrite true writing to a full buffer should block until space
	 *            is available, false if an exception should be thrown instead.
	...
	protected class CircularCharBufferReader extends Reader {
		 ...
		 *            characters, attempting to reset the stream will fail.
		 ...
		 * at the mark. If the stream has not been marked, or more characters than the
	...
	/**
	 * Class for writing to a circular character buffer. If the buffer is full, the
	 * writes will either block until there is some space available or throw an
	 * IOException based on the CircularCharBuffer's preference.
	 * @since ostermillerutils 1.00.00
	 */
	protected class CircularCharBufferWriter extends Writer {

		/**
		 * Close the stream, flushing it first. This will cause the reader associated
		 * with this circular buffer to read its last characters once it empties the
		 * buffer. Once a stream has been closed, further write() or flush() invocations
		...
		/**
		 * Write an array of characters. If the buffer allows blocking writes, this
		 * method will block until all the data has been written rather than throw an
		 * IOException.
		 * @param cbuf Array of characters to be written
		 * @throws IOException if the stream is closed, or the write is interrupted.If
		 *             buffer does not allow blocking writes and the buffer is full. If
		 *             the exception is thrown, no data will have been written since the
		...
		/**
		 * Write a portion of an array of characters. If the buffer allows blocking
		 ...
		 * low-order bits of the given integer value; the 16 high-order bits are
		 * ignored. If the buffer allows blocking writes, this method will block until
		 * all the data has been written rather than throw an IOException.
		 * @param c number of characters to be written
		 * @throws IOException if the stream is closed, or the write is interrupted.If
		 *             buffer does not allow blocking writes and the buffer is full.
		 * @since ostermillerutils 1.00.00
		 */
		@Override
		...
		/**
		 * Write a string. If the buffer allows blocking writes, this method will block
		...
		/**
		 * Write a portion of a string. If the buffer allows blocking writes, this
		 * method will block until all the data has been written rather than throw an
		 * IOException.
		 * @param str A String
		 * @param off Offset from which to start writing characters
